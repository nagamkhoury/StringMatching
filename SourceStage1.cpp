		...
		if (start) {
			...
			if (!prevStr.empty()) {
				for (size_t i = prevStr.length() - WORD_SIZE, j = WORD_SIZE - 1; i < prevStr.length() - 1; ++i, --j) {
					...
					insertTrieAndHeap(endlineWord.c_str(), &root, minHeap);
				}
			}
			...
			for (size_t i = 0; i < str.length() - WORD_SIZE; ++i)
			{
				...
				insertTrieAndHeap(word.c_str(), &root, minHeap);
			}
			prevStr = str;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	{
		printf("The file is %s, the word lengths is %d, the number of words is %d\n", fileName.c_str(), WORD_SIZE, WORD_COUNT);
		printKMostFreq(fp, WORD_COUNT);
	}
	return 0;
}